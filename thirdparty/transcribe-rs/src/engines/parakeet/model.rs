...
impl ParakeetModel {
    ...
    pub fn create_decoder_state(&self) -> Result<DecoderState, ParakeetError> {
        ...
        // Hardcode shapes as [2, 1, 640] (batch_size=1)
        // We cannot dynamically get shapes due to ort 2.0 API changes hiding type info
        let state1 = Array::zeros((2, 1, 640));
        let state2 = Array::zeros((2, 1, 640));
