...
async fn test_search_and_report_workflow() {
    ...
    let fs_tool: Box<dyn Tool> = Box::new(FilesystemTool::new(workspace));
    let search_tool: Box<dyn Tool> = Box::new(SearchTool);
    let office_tool: Box<dyn Tool> = Box::new(OfficeTool);
