...
pub fn load_skill_from_zip(zip_path: &Path) -> Result<LoadedSkill, String> {
    ...
    for i in 0..archive.len() {
        ...
        if let Err(_) = file.read_to_string(&mut content) {
            // Binary file, skip for now (could be handled differently)
            continue;
        }
...
fn detect_file_type(filename: &str) -> String {
    ...
    if lowercase.ends_with(".py") {
    ...
    } else if lowercase.ends_with(".sh") || lowercase.ends_with(".bash") || lowercase.ends_with(".zsh") {
        "shell".to_string()
        ...
        "sql".to_string()
    } else if lowercase.ends_with(".j2") || lowercase.ends_with(".jinja") || lowercase.ends_with(".jinja2") {
        "jinja".to_string()
...
"#;
    ...
    fn test_load_from_zip() {
        ...
        // Add script
        zip.start_file("test-skill/scripts/run.py", options).unwrap();
        zip.write_all(b"print('zip')").unwrap();

        zip.finish().unwrap();

        let result = load_skill_from_zip(&zip_path).unwrap();

        assert_eq!(result.skill.name, "zip-skill");
        assert!(result.files.contains_key("scripts/run.py"));
        assert_eq!(result.files["scripts/run.py"].content, "print('zip')");
    }
}
