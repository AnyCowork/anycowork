...
use crate::agents::{AgentLoop, planner::PlanningAgent};
...
impl<R: Runtime> Coordinator<R> {
    ...
    pub async fn run(&self, user_message: String) {
        ...
        let _ = Emitter::emit(&self.window, &format!("session:{}", self.session_id), AgentEvent::JobStarted {
            job: job.clone(),
        });

        // FAST MODE SHORT-CIRCUIT
        if self.mode == "fast" {
             let _ = Emitter::emit(&self.window, &format!("session:{}", self.session_id), AgentEvent::Thinking {
        ...
        let planner = PlanningAgent::new(self.agent_db.ai_model.clone(), self.agent_db.ai_provider.clone());
        
        let window_clone = self.window.clone();
        let session_id_clone = self.session_id.clone();
        
        let on_token = move |token: String| {
            let _ = Emitter::emit(&window_clone, &format!("session:{}", session_id_clone), AgentEvent::Thinking {
                message: token
            });
        };
        ...
        let plan = match planner.plan(&user_message, on_token).await {
            Ok(p) => p,
            Err(e) => {
                let _ = Emitter::emit(&self.window, &format!("session:{}", self.session_id), AgentEvent::Token { content: format!("Planning failed: {}", e) });
                return;
            }
        };

        let mut plan_update = crate::models::PlanUpdate::from(plan.clone());
        let _ = Emitter::emit(&self.window, &format!("session:{}", self.session_id), AgentEvent::PlanUpdate {
        ...
        // Finalize
        let _ = Emitter::emit(&self.window, &format!("session:{}", self.session_id), AgentEvent::JobCompleted {
